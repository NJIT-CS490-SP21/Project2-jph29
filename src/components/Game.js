  ...
  const handleClick = (i) => {
    ...
    if (inputRef != null) {
      socket.emit("game", {
        ...
        boardCopy: boardCopy /*current board*/,
        i: i /*index*/,
        xO: xO,
      ...
      <div>
        <div>
          {currUser === "" ? (
            <div>
              {" "}
              Enter Username Here (Username is case sensitive):{" "}
              <input ref={userRef} type="text" />
              <button onClick={updateUsers}>Submit</button>
            </div>
          ) : (
            <div>
              <h2>
